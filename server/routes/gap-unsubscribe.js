...
// Route: /api/gap-unsubscribe
// Feature: Unsubscribe
// Description: Unsubscribe / preference center surface
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Unsubscribe. Unsubscribe / preference center surface' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'unsubscribe', title: 'Unsubscribe', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'unsubscribe', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['unsubscribe']);
      return res.json({ slug: 'unsubscribe', items: r.rows || [] });
    }
    return res.json({ slug: 'unsubscribe', items: [] });
  } catch (e) {
    return res.json({ slug: 'unsubscribe', items: [], error: (e && e.message) || 'Server error' });
  }
});
