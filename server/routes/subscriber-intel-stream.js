// Auto-generated feature route: subscriber-intel-stream
// Domain: subscriber intelligence stream — Capability: predicting churn and auto-segmenting
// Mounted under: /api/subscriber-intel-stream
...
// POST /api/subscriber-intel-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a subscriber intelligence stream. Your task is predicting churn and auto-segmenting. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/subscriber-intel-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/subscriber-intel-stream/:id — fetch one
...
// DELETE /api/subscriber-intel-stream/:id — discard a run
