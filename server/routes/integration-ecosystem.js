// Auto-generated feature route: integration-ecosystem
// Domain: newsletter integration agent — Capability: syncing CRM + ecommerce for conversion tracking
// Mounted under: /api/integration-ecosystem
...
// POST /api/integration-ecosystem/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a newsletter integration agent. Your task is syncing CRM + ecommerce for conversion tracking. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/integration-ecosystem/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/integration-ecosystem/:id — fetch one
...
// DELETE /api/integration-ecosystem/:id — discard a run
