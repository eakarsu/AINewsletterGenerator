// Auto-generated feature route: segment-batch-test
// Domain: segment batch content tester — Capability: running auto-test and winner rollout per segment
// Mounted under: /api/segment-batch-test
...
// POST /api/segment-batch-test/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a segment batch content tester. Your task is running auto-test and winner rollout per segment. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/segment-batch-test/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/segment-batch-test/:id — fetch one
...
// DELETE /api/segment-batch-test/:id — discard a run
