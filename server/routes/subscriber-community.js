// Auto-generated feature route: subscriber-community
// Domain: subscriber community engagement agent — Capability: running engagement, commerce, networking surfaces
// Mounted under: /api/subscriber-community
...
// POST /api/subscriber-community/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a subscriber community engagement agent. Your task is running engagement, commerce, networking surfaces. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/subscriber-community/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/subscriber-community/:id — fetch one
...
// DELETE /api/subscriber-community/:id — discard a run
