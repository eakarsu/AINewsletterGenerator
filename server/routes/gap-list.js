...
// Route: /api/gap-list
// Feature: List
// Description: List import / dedupe / cleansing
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: List. List import / dedupe / cleansing' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'list', title: 'List', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'list', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['list']);
      return res.json({ slug: 'list', items: r.rows || [] });
    }
    return res.json({ slug: 'list', items: [] });
  } catch (e) {
    return res.json({ slug: 'list', items: [], error: (e && e.message) || 'Server error' });
  }
});
