// Auto-generated feature route: campaign-orchestrator-agent
// Domain: agentic newsletter orchestrator — Capability: generating content + subject + segments + schedule from a brief
// Mounted under: /api/campaign-orchestrator-agent
...
// POST /api/campaign-orchestrator-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic newsletter orchestrator. Your task is generating content + subject + segments + schedule from a brief. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
    ...
    const record = {
      ...
      slug: 'campaign-orchestrator-agent',
      input: payload,
      output: parsed,
...
// GET /api/campaign-orchestrator-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/campaign-orchestrator-agent/:id — fetch one
...
// DELETE /api/campaign-orchestrator-agent/:id — discard a run
